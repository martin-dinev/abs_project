...
class MyDQN:
    def __init__(self, state_space_shape, num_actions, model: Model, target_model, learning_rate=0.1,
    ...
    def update_memory_multiple(self, states, actions, rewards, next_states, dones):
        """
        Adds experience tuple to experience replay memory.
        :param states: current states
        :param actions: performed actions
        :param rewards: rewards received for performing actions
        :param next_states: next states
        :param dones: if episodes have terminated after performing the actions in the current states
        """
    ...
    def get_actions(self, states, epsilon):
        """
        Returns the best actions following epsilon greedy policy for the current states.
        :param states: multiple states
        :param epsilon: exploration rate
        :return:
        """
        probabilities = np.random.random(size=len(states)) + epsilon / self.num_actions
        actions = np.random.randint(0, self.num_actions, len(states))
        test_states = np.array([state for state, probability in zip(states, probabilities) if probability >= epsilon])
        if len(test_states) > 0:
            predictions = self.model.predict(test_states, verbose=0)
            actions[np.where(probabilities >= epsilon)] = np.argmax(predictions, axis=1)
        return actions
    ...
    def load(self, model_name, episode):
        ...
        """
        self.model.load_weights(f'models/{self.model_type}_{model_name}_{episode}.h5')
    ...
    def save(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.model.save_weights(f'models/{self.model_type}_{model_name}_{episode}.h5')

    def max_save(self, model_name):
        """
        Returns the max episode for which a save file exists
        :param model_name:
        :return:
        """
        max_episode = 0
        for file in os.listdir():
            if file.startswith(f'{self.model_type}_{model_name}_') and file.endswith('.h5'):
    ...
    def train(self):
        ...
        states = np.array(tuple(state for state, _, _, _, _ in minibatch))

        next_states = np.array(tuple(next_state for _, _, _, next_state, done in minibatch if not done))
        predictions = self.target_model.predict(next_states, verbose=0)

        max_future_q = list(range(len(minibatch)))
        not_done_counter = 0
        for i, (state, action, reward, _, done) in enumerate(minibatch):
            if done:
                max_future_q[i] = reward
            else:
                max_future_q[i] = reward + self.discount_factor * np.amax(predictions[not_done_counter])
                not_done_counter += 1

        actions = self.model.predict(states, verbose=0)
        for i in range(len(minibatch)):
            _, action, _, _, _ = minibatch[i]
            actions[i][action] = max_future_q[i]
...
class DDQN:
    ...
    def load(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.model.load_weights(f'models/ddqn_{model_name}_{episode}.h5')
    ...
    def save(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.model.save_weights(f'models/ddqn_{model_name}_{episode}.h5')
...
class DuelingDQN:
    ...
    def load(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.model.load_weights(f'models/duelingdqn_{model_name}_{episode}.h5')
    ...
    def save(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.model.save_weights(f'models/duelingdqn_{model_name}_{episode}.h5')
...
class DDPG:
    ...
    def load(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.actor.load_weights(f'models/actor_{model_name}_{episode}.h5')
        self.critic.load_weights(f'models/critic_{model_name}_{episode}.h5')
    ...
    def save(self, model_name, episode):
        ...
        :param episode: episode checkpoint
        """
        self.actor.save_weights(f'models/actor_{model_name}_{episode}.h5')
        self.critic.save_weights(f'models/critic_{model_name}_{episode}.h5')
